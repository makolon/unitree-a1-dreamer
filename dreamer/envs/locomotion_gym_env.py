...
class LocomotionGymEnv(gym.Env):
  ...
  def reset(self,
    ...
    return self._get_observation(action=None, reward=None, done=None, reset=None)
  ...
  def _get_observation(self, action, reward, done, reset):
    ...
    if action is not None:
      ...
      obs_dict['action'] = np.zeros(self.action_space.shape)
    
    if reward is not None:
      obs_dict['reward'] = reward
    else:
      obs_dict['reward'] = np.array(0.0)
    
    if done is not None:
      obs_dict['terminal'] = done
    else:
      obs_dict['terminal'] = np.array(False)
    
    if reset is not None:
      obs_dict['reset'] = np.array(False)
    else:
      obs_dict['reset'] = np.array(True)

    for key, val in sensors_dict.items():
      obs_dict[key] = val

    # observations = collections.OrderedDict(sorted(list(obs_dict.items()))) # observations: OrderedDict([('BaseDisplacement', array([0., 0., 0.])), ('IMU', array([ 0., -0.,  0.,  0.])), ('MotorAngle', array([ 0. ,  0.9, -1.8,  0. ,  0.9, -1.8,  0. ,  0.9, -1.8,  0. ,  0.9,
    return obs_dict
